 ...
 * }}}
 */
object TPCDSRun extends TPCDSRunBase with SQLQueryTestHelper with SQLHelper{
  var tpcdsDataPath = ""
  var injectStats = true
  ...
  private def runQuery(
    ...
    withSQLConf(conf.toSeq: _*) {
      try {
        ...
        print(s"${SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key} = " +
          s"${spark.conf.get(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key)}\n")
        print(s"${SQLConf.PREFER_SORTMERGEJOIN.key} = " +
          s"${spark.conf.get(SQLConf.PREFER_SORTMERGEJOIN.key)}\n")
        print(s"${SQLConf.ADAPTIVE_EXECUTION_ENABLED.key} = " +
          s"${spark.conf.get(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key)}\n")
        print(s"${SQLConf.ADAPTIVE_EXECUTION_ENABLED.key} = " +
          s"${spark.conf.get(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key)}\n")
